...
export default function PaymentHistory({ entry, onClose }) {
  ...
  const remaining = (parseFloat(entry.amount) || 0) - totalPaid
    ...
    <div style={{
      ...
      <div style={{ marginBottom: 10, padding: 8, background: 'var(--surface2)', borderRadius: 'var(--radius)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 4 }}>
          ...
          <span style={{ color: 'var(--text)', fontWeight: 500 }}>{fmt(entry.amount)} {entry.currency}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 4 }}>
          <span style={{ color: 'var(--muted)' }}>Paid:</span>
          <span style={{ color: 'var(--success)', fontWeight: 500 }}>{fmt(totalPaid)} {entry.currency}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, borderTop: '1px solid var(--border)', paddingTop: 4 }}>
          ...
          <span style={{ color: remaining > 0 ? 'var(--danger)' : 'var(--success)', fontWeight: 500 }}>{fmt(remaining)} {entry.currency}</span>
        </div>
      </div>
        ...
        <div style={{ maxHeight: 150, overflowY: 'auto' }}>
          {payments.map(p => (
            <div key={p.id} style={{
              ...
              <div style={{ flex: 1 }}>
                <span style={{ color: 'var(--text)', fontWeight: 500 }}>{fmt(p.amount)} {entry.currency}</span>
